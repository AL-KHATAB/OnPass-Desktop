    ...
    public partial class BiometricLoginControl : UserControl
    {
        ...
        private async void BiometricAuthButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    var consentResult = await UserConsentVerifier.RequestVerificationAsync("OnPass Authentication");
        ...
        private void HandleBiometricLogin(string username, byte[] encryptedPassword)
        {
            try
            {
                // Windows Hello gates access, but the actual vault still opens with
                // the original master-password-derived key after DPAPI unprotects it.
                ...
                {
                    // Biometric login must initialize the same extension session path
                    // as password login so the browser does not see a degraded flow.
